...
async def post_key(user_info: Annotated[UserRecord, Depends(authentication_jwt_middleware)], token: str, email: str, tier: int = Query(0, ge=0, le=2),
                   key_service: KeysService = Depends(get_keys_service)):
    """
    Add key
    """
    logger.info({"route": "/add-key", "message": f"Adding key for user {user_info.uid}"})
    return add_new_key(user_info, token, email, tier, key_service)
...
async def get_active_key(user_info: Annotated[UserRecord, Depends(authentication_jwt_middleware)], tier: int = Query(0, ge=0, le=2),
                         key_service: KeysService = Depends(get_keys_service)):
    """
    Get last active key
    """
    logger.info({"route": "/get-last-active-key", "message": f"Getting last active key for user {user_info.uid}"})
    return get_last_valid_key(user_info, tier, key_service)
